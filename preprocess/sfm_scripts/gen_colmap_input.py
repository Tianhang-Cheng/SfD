...
parser.add_argument("--use_original_intrinsics", action="store_true", default=False,
    help="If set, use the 60 deg FoV intrinsics mentioned in the paper. Note that this should be consistent with \
        ORB-SLAM config. Otherwise, we use the provided GT intrinsics. Default to False.")
...
# cameras.txt
img_sample = cv2.imread(os.path.join(data_dir, "image_0", "%06d.png"%0))
...
fx, fy, cx, cy = get_intrinsics(gt_fpath, width, height, args.use_original_intrinsics)
